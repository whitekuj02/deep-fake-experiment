import librosa # 음성쪽에서 대표적인 라이브러리 , front-end : LFCC
import sys 
import os 
...
from torchaudio.models import wav2vec2_base #front model wav2vec2 
from torchaudio.models import hubert_base # ㅂㅣ슷
...
import torchmetrics
import os
import argparse # 매개변수 정의 
from time import time, strftime, localtime
#from modules.get_model import get_model
from typing import Dict
from importlib import import_module # 다른 폴더에 잇는 것들 import 해줄 수 있게 ㅎ
...
DBNAME = "all_more_train" # ㄷㅔ이터 정보가 들어잇는 이름 
TNAME = "test"
SUBMIT = "/home/aicontest/DF/result/uijin" #제출 파일
...
def get_argument():
    ...
    #hyperparameter
    parser.add_argument("--config", dest="config", type=str, help="configuration file", required=True) ### 무조건 써야됨 
    ...
    parser.add_argument("-sf", "--sample_frequency", type=int, default=16000, help="Resample the waveform using sample rate") # sampling 길이를 얼마나 가져갈거냐
    # waveform을 resampling할 때 어떤 sampling frequency를 사용할 것인가 
    parser.add_argument("-svt", "--sound_vector_threshold", type=int, default=80000, help="fixed sound vector size")


    # 어떤 조합으로 사용을 할 것인가, 4번 사용 -> 3가지 방법 모두 사용, 논문에서는 5번이 SOTA
...
args = get_argument() # args 함수로 저장하기 
print("== argment 처리 완료 ==")
...
## validation 처리
if args.validation:
    train, val = train_test_split(df, test_size=args.validation_size, random_state=args.seed, stratify=df[['real', 'fake']]) # real fake label 유지하면서 
else:
    train = df
...
def pad_or_truncate(features, device, fixed_length=args.sound_vector_threshold, min_second_dim=3):
    ...
    if features.shape[0] >= fixed_length: # 정해준 threshold 보다 vector 크기 (기본데이터가) 가 더 큰지 안 큰지 판단 
        # Truncate the tensor to the fixed length
        features = features[:fixed_length] # 정해둔 길이보다 크면 잘라버림 
    else:
        # threshold 를 못넘었을 때 padding을 해줘야됨 

        # Repeat the tensor to the fixed length
        num_repeats = (fixed_length // features.shape[0]) + 1 # 몇 번 반복해서 붙여줘야지 threshold를 넘는 지 check 
        padded_features = torch.tensor([], device=device)

        for _ in range(num_repeats):
            padded_features = torch.cat((padded_features, features), dim=0)

        #print(f"누가 범인인가 {padded_features.shape[0]}")
        features = padded_features[:fixed_length]  # 정해둔 길이보다 크면 잘라버림  
...
def front_end(df, train_mode=True):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        # 이부분에 frontend 삽입
        # torchaudio 로 데이터로드 , waveform -> 1차원 row data, sample_rate 파일을 만들 때 사용했던 sample rate , 32000
        waveform, sample_rate = torchaudio.load(args.db_dir + '/' + row['path'])
        
        resampler = torchaudio.transforms.Resample(orig_freq=sample_rate, new_freq=args.sample_frequency) # 기존 sample rate를 우리가 설정한 sample rate로 변경해준다 

        waveform = resampler(waveform) 

        waveform = pad_or_truncate(waveform.squeeze(0).to(device), device) #input vector 사이즈 맞춰주기 
        #[[13, 13, 32, 51]] (1,4)  =>  shape = [ 13, 13, 32, 51 ] (4) , 
        ...
        ### raw 데이터를 모델 input으로 잘 넣어줄 수 있게 가공 완료~~~~~~~~

    if train_mode:
        return features, labels
    return features

train_data, train_labels = front_end(train, True) # feature 추출 
...
class CustomDataset(Dataset):
    def __init__(self, data, label, args, algo=4, noise_rate=0.1, noise_probability=0.1):
        self.data = data # waveform
        self.label = label # real, fake 
        self.args = args #위에서 받은 모든 args 
        self.algo = algo 
    ...
    def __getitem__(self, index): 
        ...
        if self.label is not None: # train 데이터일 때 
...
train_dataset = CustomDataset(train_data, train_labels, args, 4)
...
if args.validation:
    val_dataset = CustomDataset(val_data, val_labels, args, 4)
...
def train(model, optimizer, train_loader, device, val_loader=False):
    ...
    for epoch in range(1, args.n_epoch+1):
        ...
        for features, labels in tqdm(iter(train_loader), total=len(train_loader)):
            ...
            #output = output[1]
            output = torch.sigmoid(output[1]) #0~1사이의 확률 값으로 나타내줘야하기 때문에 , output이 확률로 나와야되기 때문에 
            # output[0] ->hidden state, output[1] -> output / RNN 느낌 
